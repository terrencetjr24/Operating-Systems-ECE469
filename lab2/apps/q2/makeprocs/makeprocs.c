...
void main (int argc, char *argv[])
{
  ...
  for(i=0; i<numprocs; i++) {
    process_create(PRODUCER_TO_RUN, h_mem_str, s_procs_completed_str, l_buff_str, NULL);
    Printf("Producer %d created\n", i);
  }

  for(i=0; i<numprocs; i++) {
    process_create(CONSUMER_TO_RUN, h_mem_str, s_procs_completed_str, l_buff_str, NULL);
